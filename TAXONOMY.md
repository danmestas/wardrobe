...
| `outfits/` & `accessories/` | The bundle layer. Outfits are long-lived role bundles (e.g. `backend`, `frontend`); accessories are piecemeal add-ons applied via `--accessory`. Both reference flat `skills/`, `agents/`, `hooks/`, `rules/`, `commands/` entries by name. |
| `README.md` catalog | Components grouped by `primary` category inside the marker-bounded auto-region. Regenerated by `suit-build docs` once the `category:` field lands. |
| `suit.config.yaml` | Reshape from flat `<harness>.<key>` to `<harness>.<category>.<key>` slices. Per-category overrides become first-class. |
| Audit | `suit-build validate --category <name>` lists components carrying that category (primary or secondary) — see Section 5. |
...
Once the `category:` field lands in frontmatter and is validated by the schema, `suit-build` exposes a category-aware audit:

```bash
suit-build validate --category economy
```
